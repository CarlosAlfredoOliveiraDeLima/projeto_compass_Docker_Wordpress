...
WP_VPC_ID=$(aws ec2 create-vpc --cidr-block 172.16.0.0/16 --query="Vpc.VpcId" \
        --tag-specifications "ResourceType=vpc,Tags=[{Key=Name,Value=wordpress-task-vpc},$TAGS]")
...
WP_PRIV_SUBNET_1=$(aws ec2 create-subnet --vpc-id "$WP_VPC_ID" --availability-zone us-east-1a --cidr-block 172.16.0.0/22 --query="Subnet.SubnetId" \
            --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-priv-subnet-1},$TAGS]")
...
WP_PRIV_SUBNET_2=$(aws ec2 create-subnet --vpc-id "$WP_VPC_ID" --availability-zone us-east-1b --cidr-block 172.16.4.0/22 --query="Subnet.SubnetId" \
            --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-priv-subnet-2},$TAGS]")
...
WP_PUB_SUBNET_1=$(aws ec2 create-subnet --vpc-id "$WP_VPC_ID" --availability-zone us-east-1a --cidr-block 172.16.250.0/24 --query="Subnet.SubnetId" \
            --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-pub-subnet-1},$TAGS]")
...
WP_PUB_SUBNET_2=$(aws ec2 create-subnet --vpc-id "$WP_VPC_ID" --availability-zone us-east-1b --cidr-block 172.16.251.0/24 --query="Subnet.SubnetId" \
            --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-pub-subnet-2},$TAGS]")
...
WP_RDS_PRIV_SUBNET_1=$(aws ec2 create-subnet --vpc-id "$WP_VPC_ID" --availability-zone us-east-1a --cidr-block 172.16.254.0/28 --query="Subnet.SubnetId" \
            --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-rds-priv-subnet-1},$TAGS]")
...
WP_RDS_PRIV_SUBNET_2=$(aws ec2 create-subnet --vpc-id "$WP_VPC_ID" --availability-zone us-east-1b --cidr-block 172.16.254.16/28 --query="Subnet.SubnetId" \
            --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-rds-priv-subnet-2},$TAGS]")
...
WP_IGW_ID=$(aws ec2 create-internet-gateway --query="InternetGateway.InternetGatewayId" \
        --tag-specifications "ResourceType=internet-gateway,Tags=[{Key=Name,Value=wordpress-task-igw},$TAGS]")
...
WP_NATGW_PUB_IP_ID=$(aws ec2 allocate-address --query="AllocationId" \
                    --tag-specifications "ResourceType=elastic-ip,Tags=[{Key=Name,Value=wordpress-task-natgw-pub-ip},$TAGS]")
...
WP_NATGW_ID=$(aws ec2 create-nat-gateway --subnet-id "$WP_PUB_SUBNET_1" --allocation-id "$WP_NATGW_PUB_IP_ID" \
            --connectivity-type public --query="NatGateway.NatGatewayId" \
            --tag-specifications "ResourceType=natgateway,Tags=[{Key=Name,Value=wordpress-task-natgw},$TAGS]")
...
aws ec2 create-tags --resources "$WP_PRIV_RTB_ID" \
    --tags '[{"Key":"Name","Value":"wordpress-task-priv-rtb"},{"Key":"Task","Value":"Docker-WordPress"},{"Key":"Project","Value":"PB UNIVESP URI"},{"Key":"CostCenter","Value":"C092000004"}]'
...
WP_PUB_RTB_ID=$(aws ec2 create-route-table --vpc-id "$WP_VPC_ID" --query="RouteTable.RouteTableId" \
            --tag-specifications "ResourceType=route-table,Tags=[{Key=Name,Value=wordpress-task-pub-rtb},$TAGS]")
...
do
aws ec2 associate-route-table --route-table-id "$WP_PRIV_RTB_ID" --subnet-id "$subnet" > /dev/null
done
echo "Private subnets route table associations done"

for subnet in $WP_PUB_SUBNET_1 $WP_PUB_SUBNET_2
do
aws ec2 associate-route-table --route-table-id "$WP_PUB_RTB_ID" --subnet-id "$subnet" > /dev/null
done

...
VPC_ID=$(aws ec2 create-vpc --cidr-block 172.16.0.0/16 --query="Vpc.VpcId"  --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=wordpress-task-vpc}]')
aws ec2 modify-vpc-attribute --enable-dns-hostnames --vpc-id "$VPC_ID"
echo "VPC <$VPC_ID> created"

# Creates all subnets to be used in the project
PRIV_SUBNET_1=$(aws ec2 create-subnet --vpc-id "$VPC_ID" --availability-zone us-east-1a --cidr-block 172.16.0.0/22 --query='Subnet.SubnetId'  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-priv-subnet-1}]')
echo "Private subnet 1 <$PRIV_SUBNET_1> created"
PRIV_SUBNET_2=$(aws ec2 create-subnet --vpc-id "$VPC_ID" --availability-zone us-east-1b --cidr-block 172.16.4.0/22 --query='Subnet.SubnetId'  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=wordpress-task-priv-subnet-2}]')
...
IGW_ID=$(aws ec2 create-internet-gateway --query='InternetGateway.InternetGatewayId'  --tag-specifications 'ResourceType=internet-gateway,Tags=[{Key=Name,Value=wordpress-task-igw}]')
aws ec2 attach-internet-gateway --internet-gateway-id "$IGW_ID" --vpc-id "$VPC_ID"
echo "Internet Gateway <$IGW_ID> created"

# Allocates an Elastic IP for the NAT Gateway
NATGW_PUB_IP_ID=$(aws ec2 allocate-address --query='AllocationId'  --tag-specifications 'ResourceType=elastic-ip,Tags=[{Key=Name,Value=wordpress-task-natgw-pub-ip}]')
...
NATGW_ID=$(aws ec2 create-nat-gateway --subnet-id "$PUB_SUBNET_1" --allocation-id "$NATGW_PUB_IP_ID" --connectivity-type public --query='NatGateway.NatGatewayId'  --tag-specifications 'ResourceType=natgateway,Tags=[{Key=Name,Value=wordpress-task-natgw}]')
while [[ $(aws ec2 describe-nat-gateways --nat-gateway-ids "$NATGW_ID" --query 'NatGateways[0].State' ) != "available" ]]
do
sleep 10
done
...
PRIV_RTB_ID=$(aws ec2 describe-route-tables --filters "Name=vpc-id,Values=$VPC_ID" "Name=association.main,Values=true" --query 'RouteTables[*].RouteTableId' )
...
PUB_RTB_ID=$(aws ec2 create-route-table --vpc-id "$VPC_ID" --query='RouteTable.RouteTableId'  --tag-specifications 'ResourceType=route-table,Tags=[{Key=Name,Value=wordpress-task-pub-rtb}]')
